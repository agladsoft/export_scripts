...
HEADERS_ENG: dict = {
    ...
    ("УНИ-Заказчик (есть вопросы по унификации; сейчас убрали кавычки, "
     "ОПФ привели в сокращенный вид, все - в верхний регистр)",): "customer",
    ("Наименование груза (унифицированное)",): "goods_name",
    ("Отправитель (НА ПЕРВОМ ЭТАПЕ ПОКА ПОДГРУЖАЕМ ТО, ЧТО ЕСТЬ - после создания таблицы "
     "соединяем в представлении с REFERENCE_INN)",): "shipper_name",
    ("Получатель (оставляем как есть)",): "consignee_name",
    ("Вид транспорта (оставляем как есть)",): "transport_type",
    ("Количество, тонн",): "goods_weight_tonne",
    ("Местонахождение (унифицируем чеез парсинг ИНН и получаем 3 поля: ИНН / НАИМЕНОВАНИЕ / АДРЕС",): "location",
    ("Отправитель адрес (оставляем как есть)",): "shipper_country",
    ("Получатель адрес (унифицированное)",): "consignee_country"
}
